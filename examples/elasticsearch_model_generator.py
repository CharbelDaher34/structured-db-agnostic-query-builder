    ...
    if "results" in result:
        ...
        if res["documents"]:
            ...
            for i, doc in enumerate(res["documents"][:10], 1):
                ...
                print(
                    f"     Amount: {doc.get('transaction', {}).get('amount', 0):,} {doc.get('transaction', {}).get('currency', 'N/A')}"
                )
                print(
                    f"     Location: {doc.get('transaction', {}).get('receiver', {}).get('location', 'N/A')}"
                )
    ...
    if "results" in result:
        ...
        if res["documents"]:
            ...
            for i, doc in enumerate(res["documents"][:15], 1):
                ...
                card_type = (
                    doc.get("key", {}).get("card_type", "N/A")
                    if isinstance(doc.get("key"), dict)
                    else doc.get("card_type", "N/A")
                )
                txn_type = (
                    doc.get("key", {}).get("transaction.type", "N/A")
                    if isinstance(doc.get("key"), dict)
                    else doc.get("transaction.type", "N/A")
                )

                total = (
                    doc.get("total_amount", {}).get("value", 0)
                    if isinstance(doc.get("total_amount"), dict)
                    else doc.get("total_amount", 0)
                )
                avg = (
                    doc.get("avg_amount", {}).get("value", 0)
                    if isinstance(doc.get("avg_amount"), dict)
                    else doc.get("avg_amount", 0)
                )
