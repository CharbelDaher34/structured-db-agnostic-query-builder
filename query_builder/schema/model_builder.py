...
class ModelBuilder:
    ...
    def build(self, model_name: str = "GeneratedModel") -> type[BaseModel]:
        ...
        if self._model_class is None:
            self._model_class = self._build_pydantic_model(self.schema, model_name)
        return self._model_class

    # Schema-side keys we preserve into model_info so adapter-specific
    # translators can read them (e.g. ES needs `es_type` to know whether to
    # add a `.keyword` suffix).
    _PRESERVED_SCHEMA_KEYS = ("es_type", "has_keyword_subfield", "mongo_type")
    ...
    def get_model_info(self) -> dict[str, Any]:
        ...
        if self._model_info is None:
            ...
            # Merge adapter-specific metadata from the original schema so
            # translators downstream can see it.
            for field_path, schema_entry in self.schema.items():
                extra = {
                    k: schema_entry[k] for k in self._PRESERVED_SCHEMA_KEYS if k in schema_entry
                }
                if extra and field_path in info:
                    info[field_path].update(extra)
    ...
    def _build_pydantic_model(
        ...
        # Auto-promote implicit parents. If the schema only has dotted paths
        # (e.g. `user.name`) without an explicit `user` entry, create one so we
        # still build a nested model instead of silently dropping the children.
        for parent in grouped_fields:
            if parent not in direct_fields:
                direct_fields[parent] = {"type": "object"}
    ...
    def _create_enum_type(self, field_name: str, model_name: str, values: list[Any]) -> type[Enum]:
        ...
        for i, value in enumerate(values):
            ...
            if isinstance(value, str):
                ...
                member_name = (
                    value.replace(" ", "_").replace("-", "_").replace("'", "").replace(".", "_")
                )
                if not member_name or (not member_name[0].isalpha() and member_name[0] != "_"):
                    member_name = f"_{member_name}"
                member_name = (
                    "".join(c for c in member_name if c.isalnum() or c == "_") or f"VALUE_{i}"
                )
            else:
    ...
    def _extract_model_info(self, model_class: type[BaseModel], prefix: str = "") -> dict[str, Any]:
