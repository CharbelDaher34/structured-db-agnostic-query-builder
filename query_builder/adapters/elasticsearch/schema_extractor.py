...
class ESSchemaExtractor:
    ...
    def __init__(
        ...
        Args:
            ...
            client: Optional shared Elasticsearch client. When provided, this
                extractor does not own it (won't close on cleanup). Pass one in
                to share a single connection pool with the executor.
        """
    ...
    def close(self) -> None:
        """Close the ES client if owned by this extractor."""
    ...
    def extract_schema(self) -> dict[str, Any]:
        ...
        self._schema_cache = self._normalize_mapping(self._mapping_cache or {}, "")
    ...
    def _normalize_mapping(self, mapping: dict[str, Any], prefix: str) -> dict[str, Any]:
        """
        ...
        for field_name, field_props in mapping.items():
            ...
            # Track the original ES type so the translator can decide whether
            # to suffix `.keyword` (only `text` fields need it; `keyword`
            # fields are exact-match already).
            field_info: dict[str, Any] = {
                "type": normalized_type,
                "es_type": es_type,
            }
            # Surface explicit `.keyword` sub-fields so we don't suffix blindly.
            sub_fields = field_props.get("fields") or {}
            if any(sub.get("type") == "keyword" for sub in sub_fields.values()):
                field_info["has_keyword_subfield"] = True

            # Handle nested objects
            if "properties" in field_props:
                nested_schema = self._normalize_mapping(field_props["properties"], full_path)
    ...
    def get_distinct_values(self, field_path: str, size: int = 1000) -> list[Any]:
        ...
        try:
            mappings = self.es_client.indices.get_mapping(index=self.index_name)
            index_mapping = (
                mappings.get(self.index_name, {}).get("mappings", {}).get("properties", {})
            )
            ...
            if nested_path:
                query = {
                    ...
                    "aggs": {
                        "nested_agg": {
                            ...
                            "aggs": {
                                "distinct_values": {"terms": {"field": agg_field, "size": size}}
                            },
                        }
                    },
                }
                ...
                query = {
                    "size": 0,
                    "aggs": {"distinct_values": {"terms": {"field": agg_field, "size": size}}},
                }
                response = self.es_client.search(index=self.index_name, **query)
                buckets = (
                    response.get("aggregations", {}).get("distinct_values", {}).get("buckets", [])
                )
